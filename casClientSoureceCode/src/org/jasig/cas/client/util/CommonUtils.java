...
public final class CommonUtils {
     ...
     */
    public static String constructRedirectUrl(String casServerLoginUrl, final String serviceParameterName, String serviceUrl, final boolean renew, final boolean gateway) {
        try {
/*	   System.out.println(casServerLoginUrl);
	   System.out.println(serviceParameterName);
	   System.out.println(serviceUrl);
	   System.out.println(renew);
	   System.out.println(gateway);
*/     String[] casServerLoginUrlArr = casServerLoginUrl.split(",");
       String[] serviceUrlArr = serviceUrl.split(",");
       if(AuthenticationFilter.isInnerNet){
           casServerLoginUrl = casServerLoginUrlArr[0];
           if(serviceUrlArr.length>2){
               serviceUrl = serviceUrlArr[0] + serviceUrlArr[2];  
           }else{
        	   serviceUrl = serviceUrlArr[0];
           }
       }else{
           casServerLoginUrl = casServerLoginUrlArr[1];
           if(serviceUrlArr.length>2){
               serviceUrl = serviceUrlArr[1] + serviceUrlArr[2];  
           }else if(serviceUrlArr.length==1){
        	   serviceUrl = serviceUrlArr[0];
           }else{
        	   serviceUrl = serviceUrlArr[1];
           }
       }
       String aa = casServerLoginUrl + (casServerLoginUrl.indexOf("?") != -1 ? "&" : "?") + serviceParameterName + "="
...
/**
    ...
    public static String constructServiceUrl(final HttpServletRequest request,
        ...
        String serverNameCopy = serverName;

        if(serverName!=null){
        	String[] serverArr = serverName.split(",");
            if(AuthenticationFilter.isInnerNet){
                if(serverArr.length>2){
                	serverNameCopy = serverArr[0] + serverArr[2];  
                }else{
                	serverNameCopy = serverArr[0];
                }
            }else{
                if(serverArr.length>2){
                	serverNameCopy = serverArr[1] + serverArr[2];  
                }else{
                	serverNameCopy = serverArr[1];
                }
            }
        }
        
       /* System.out.println(serverName);
        System.out.println(serverNameCopy);
        System.out.println(artifactParameterName);
*/

        if (!serverNameCopy.startsWith("https://") && !serverNameCopy.startsWith("http://")) {
            buffer.append(request.isSecure() ? "https://" : "http://");
        }

        buffer.append(serverNameCopy);
