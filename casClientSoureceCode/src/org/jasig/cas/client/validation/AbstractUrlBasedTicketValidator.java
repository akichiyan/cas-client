...
public abstract class AbstractUrlBasedTicketValidator implements TicketValidator {
    ...
    protected final String constructValidationUrl(final String ticket, final String serviceUrl) {
    	...
    	String casServerUrlAdd = null,casServerUrlPrefixAdd = null;
        String[] casServerLoginUrlArr = serviceUrl.split(",");
        String[] casServerUrlPrefixArr = casServerUrlPrefix.split(",");
        if(casServerLoginUrlArr.length>1){
        	if(AuthenticationFilter.isInnerNet){
                if(casServerLoginUrlArr.length>2){
                	casServerUrlAdd = casServerLoginUrlArr[0] + casServerLoginUrlArr[2];  
                }else{
                	casServerUrlAdd = casServerLoginUrlArr[0];
                }
            }else{
                if(casServerLoginUrlArr.length>2){
                	casServerUrlAdd = casServerLoginUrlArr[1] + casServerLoginUrlArr[2];  
                }else{
                	casServerUrlAdd = casServerLoginUrlArr[1];
                }
            } 
        }else{
        	casServerUrlAdd = serviceUrl;
        }
        
        
        if(casServerUrlPrefixArr.length>1){
        	if(AuthenticationFilter.isInnerNet){
                if(casServerUrlPrefixArr.length>2){
                	casServerUrlPrefixAdd = casServerUrlPrefixArr[0] + casServerUrlPrefixArr[2];  
                }else{
                	casServerUrlPrefixAdd = casServerUrlPrefixArr[0];
                }
            }else{
                if(casServerUrlPrefixArr.length>2){
                	casServerUrlPrefixAdd = casServerUrlPrefixArr[1] + casServerUrlPrefixArr[2];  
                }else{
                	casServerUrlPrefixAdd = casServerUrlPrefixArr[1];
                }
            } 
        }else{
        	casServerUrlPrefixAdd = casServerUrlPrefix;
        }
        
        log.debug("Placing URL parameters in map.");
        urlParameters.put("ticket", ticket);
        urlParameters.put("service", encodeUrl(casServerUrlAdd));
        ...
        final StringBuilder buffer = new StringBuilder(urlParameters.size()*10 + casServerUrlPrefixAdd.length() + suffix.length() +1);

        int i = 0;

        buffer.append(casServerUrlPrefixAdd);
        if (!casServerUrlPrefixAdd.endsWith("/")) {
            buffer.append("/");
        }
        ...
        //System.out.println("constructValidationUrl==return=="+buffer.toString());
